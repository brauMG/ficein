@extends('layouts.app', ['activePage' => 'Constancias de Inversión', 'titlePage' => __('Constancias de Inversión')])
                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="container message-box bg-danger">
                            <h6 class="own-card-title">Verificar Constancias de Inversión</h6>
                ...
                <div class="row mt-4">
                    <div class="col-md-8">
                        <div class="container message-box bg-transparent">
                            <form action="{{route('verificar_constancias_inversiones')}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-row">
                                ...
                                <table class="table table-striped  data-table">
                                    ...
                                    @foreach ($constancias_inversiones as $constancia)
                                        <tr>
                                            ...
                                            <td>
                                                @if(date('m', strtotime($constancia->date)) === '01')
                                                    Enero
                                                @elseif(date('m', strtotime($constancia->date)) === '02')
                                                    Febrero
                                                @elseif(date('m', strtotime($constancia->date)) === '03')
                                                    Marzo
                                                @elseif(date('m', strtotime($constancia->date)) === '04')
                                                    Abril
                                                @elseif(date('m', strtotime($constancia->date)) === '05')
                                                    Mayo
                                                @elseif(date('m', strtotime($constancia->date)) === '06')
                                                    Junio
                                                @elseif(date('m', strtotime($constancia->date)) === '07')
                                                    Julio
                                                @elseif(date('m', strtotime($constancia->date)) === '08')
                                                    Agosto
                                                @elseif(date('m', strtotime($constancia->date)) === '09')
                                                    Septiembre
                                                @elseif(date('m', strtotime($constancia->date)) === '10')
                                                    Octubre
                                                @elseif(date('m', strtotime($constancia->date)) === '11')
                                                    Noviembre
                                                @elseif(date('m', strtotime($constancia->date)) === '12')
                                                    Diciembre
                                                @endif
                                            </td>
                                            <td>{{date('Y', strtotime($constancia->date))}}</td>
                                            <td class="action-row">
                                                <a href="{{ url('/cliente/constancia_inversion/pdf/download/' . $constancia->id) }}" rel="tooltip" class="btn btn-sm btn-warning btn-adjust">
